...
class PexelsBackground {
    ...
    useFallbackBackground() {
        ...
        // No Pexels API key configured: use a bundled static photo instead of
        // fetching from the network, so the hero still has a real background
        // (matches the theme's own applyBackgroundImage() treatment).
        const staticBg = '/images/hero-bg.jpg';
        if (staticBg) {
            heroSection.style.background = 'transparent';
            heroSection.classList.remove('gradient-bg');
            heroSection.classList.add('pexels-bg');

            let bgContainer = heroSection.querySelector('.pexels-bg-container');
            if (!bgContainer) {
            ...
            bgContainer.style.backgroundImage = `url(${staticBg})`;
            bgContainer.style.backgroundSize = 'cover';
            bgContainer.style.backgroundPosition = 'center';
            bgContainer.style.backgroundRepeat = 'no-repeat';
            ...
            this.setCredit({ photographer: 'Brett Sayles', url: 'https://www.pexels.com/photo/server-racks-on-data-center-4508751/' });
            return;
        }

        // Last-resort fallback: flat gradient (no static image configured either).
